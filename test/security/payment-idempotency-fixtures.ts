...
export async function createTenant(prisma: PrismaClient) {
  return prisma.tenant.create({
    data: {
      name: `RC-001 tenant ${randomUUID()}`,
      slug: `rc001-${randomUUID()}`,
    },
  });
}
...
export async function createConnection(
  ...
  return prisma.paymentConnection.create({
    data: {
      tenantId: tenant.id,
      providerCode: PaymentProviderCode.MERCADO_PAGO,
      displayName: 'RC-001 fake provider',
      status: PaymentConnectionStatus.ACTIVE,
      encryptedCredentials: 'fake-encrypted-credentials',
      capabilities: ['PIX'],
