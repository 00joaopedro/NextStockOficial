...
import { localJwtConfig } from './local-jwt-config';

export type LocalOnlyEvidenceSource =
  | 'synthetic'
  | 'staging'
  | 'production'
  | 'unknown';
...
export function evaluateLocalOnlyReadiness(
  ...
  if (env.AUTH_PROVIDER_MODE !== 'local_only')
    blockers.push('LOCAL_ONLY_NOT_SELECTED');
  if (env.AUTH_LOCAL_ONLY_CONFIRMATION !== 'true')
    blockers.push('EXPLICIT_LOCAL_ONLY_CONFIRMATION_REQUIRED');
  ...
  if (legacyFallbackEnabled(env))
    blockers.push('LEGACY_FALLBACK_MUST_BE_DISABLED');
  try {
    localJwtConfig(env);
  } catch {
    blockers.push('LOCAL_JWT_CONFIGURATION_INVALID');
  }
  ...
  if (env.AUTH_PREFLIGHT_UNKNOWN_STATES === undefined)
    blockers.push('AUTH_UNKNOWN_STATE_EVIDENCE_UNKNOWN');
  else if (env.AUTH_PREFLIGHT_UNKNOWN_STATES !== '0')
    blockers.push('AUTH_UNKNOWN_STATES_PRESENT');
  if (env.AUTH_LOCAL_PRIMARY_OBSERVED !== 'true')
    blockers.push('LOCAL_PRIMARY_OBSERVATION_REQUIRED');
  if (source === 'unknown') blockers.push('EVIDENCE_SOURCE_UNKNOWN');
  if (source === 'synthetic')
    warnings.push('SYNTHETIC_EVIDENCE_NOT_PRODUCTION_PROOF');
  if (source === 'production')
    blockers.push('PRODUCTION_EVIDENCE_NOT_ALLOWED_FOR_LOCAL_PREPARATION');
  return {
    status: blockers.length
      ? missingEvidence || source === 'unknown'
        ? 'UNKNOWN'
        : 'NOT_READY'
      : 'READY',
...
function parseSource(value: string | undefined): LocalOnlyEvidenceSource {
  if (value === 'synthetic' || value === 'staging' || value === 'production')
    return value;
  return 'unknown';
}
