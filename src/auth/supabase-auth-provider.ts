...
export class SupabaseAuthProvider implements AuthIdentityProvider {
  ...
  async completePasswordRecovery(input: { accessToken: string; refreshToken: string; newPassword: string }) {
    const { data: sessionData, error: sessionError } = await this.supabase.anon.auth.setSession({ access_token: input.accessToken, refresh_token: input.refreshToken });
    if (sessionError || !sessionData.user || !sessionData.session) throw this.error(sessionError, 'invalid_credentials');
    const { data, error } = await this.supabase.anon.auth.updateUser({ password: input.newPassword });
    if (error || !data.user) throw this.error(error, 'invalid_credentials');
    await this.supabase.anon.auth.signOut({ scope: 'global' });
