...
describe('local password policy', () => {
  it('accepts spaces, symbols and unicode without composition rules', () => {
    expect(() => validateLocalPassword('Senha local segura! 你好')).not.toThrow();
  });
  ...
  it('rejects short, control-character and bcrypt-overlong passwords', () => {
    ...
    expect(() => validateLocalPassword(`senha segura ${String.fromCharCode(0)}`)).toThrow('PASSWORD_INVALID');
    expect(() => validateLocalPassword('a'.repeat(73))).toThrow('PASSWORD_TOO_LONG_FOR_BCRYPT');
  });
  ...
  it('hashes and compares without exposing the password', async () => {
    ...
    await expect(hasher.compare('Senha local segura!', hash)).resolves.toBe(true);
