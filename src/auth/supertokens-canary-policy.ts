...
export function readSuperTokensCanaryPolicy(env: NodeJS.ProcessEnv = process.env): CanaryPolicy {
  ...
  if (killSwitch && enabled) throw new Error('AUTH_CANARY_KILL_SWITCH conflicts with AUTH_CANARY_ENABLED');
  return { enabled, killSwitch, allowlist, percentage: parsePercentage(env.AUTH_CANARY_PERCENTAGE) };
}

export function decideSuperTokensCanary(subject: string, policy: CanaryPolicy): CanaryDecision {
  ...
  const bucket = createHash('sha256').update(subject, 'utf8').digest().readUInt32BE(0) % 100;
