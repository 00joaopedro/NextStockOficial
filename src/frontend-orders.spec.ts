...
describe('orders frontend production flow', () => {
  it('pedido.html usa script real e deixa DEMO_ORDERS restrito ao modo demo', () => {
    ...
    const script = readFileSync(join(root, 'public', 'Js', 'pedido.js'), 'utf8');
  ...
  it('ntfe.html busca rascunho fiscal por orderId em vez de depender do pedido inteiro no sessionStorage', () => {
    ...
    expect(html).toContain('/api/orders/${encodeURIComponent(orderId)}/nfe-draft');
    expect(html).toContain('const orderId = new URLSearchParams(window.location.search).get("orderId")');
