...
export function evaluateSuperTokensGates(input: SuperTokensGateInput) {
  ...
  if (input.reconciliationRequired > 0)
    blockers.push('AUTH_RECONCILIATION_REQUIRED');
  if (input.duplicateCanonicalEmails > 0)
    blockers.push('AUTH_CANONICAL_EMAIL_DUPLICATE');
  if (input.unresolvedPasswordStrategies > 0)
    blockers.push('AUTH_PASSWORD_STRATEGY_UNRESOLVED');
  if (input.legacySessionsOutsideWindow > 0)
    blockers.push('AUTH_LEGACY_SESSIONS_OUTSIDE_WINDOW');
