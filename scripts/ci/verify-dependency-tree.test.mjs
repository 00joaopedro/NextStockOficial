...
test('problem order and duplicates do not affect collection', () => { const result = collectProblems({ ...tree(), problems: ['extraneous: @emnapi/runtime@1.11.1', 'extraneous: @img/sharp-wasm32@0.35.2'] }); assert.equal(result.length >= 4, true); assert.equal(validateKnownSharpWasmArtifacts({ npmTree: tree({ reverse: true }), lockfile, installedPackages: installed }).accepted, true); });
