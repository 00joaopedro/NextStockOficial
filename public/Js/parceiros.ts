...
function operationalHeaders(): Record<string, string> {
  try {
...
async function loadPartners() {
  ...
  try {
    ...
    els.totalReferredUsers.textContent = String(
      data.metrics.totalReferredUsers,
    );
...
async function updateSeen(input: HTMLInputElement) {
  ...
  try {
    await api(
      `/partners/${state.selected.id}/referrals/${input.dataset.seen}/seen`,
      {
        method: 'PATCH',
...
els.partnerList.addEventListener('click', (event) => {
  ...
  const partner = state.partners.find(
    (item) => item.id === card?.dataset.partnerId,
  );
